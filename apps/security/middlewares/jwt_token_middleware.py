...
from apps.constants.error_messages import TOKEN_EXPIRED


class JWTAuthMiddleware(MiddlewareMixin):
    def process_request(self, request: Request, **kwargs):
        access_token: Token = request.COOKIES.get('access_token')
        refresh_token: Token = request.COOKIES.get('refresh_token')
        ...
        if access_token:
            try:
                ...
                if new_access_token:
                    ...
                    self.remove_jwt_cookies(request)
